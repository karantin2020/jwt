...
func TestDecodeToken(t *testing.T) {
	...
	if assert.NoError(t, err, "Error parsing signed token.") {
		assert.Error(t, tok3.Claims(sharedKey, &StandardClaims{}), "Expected unmarshaling claims to fail.")
	}
	...
	if assert.NoError(t, err, "Error parsing encrypted token.") {
		assert.Error(t, tok6.Claims(sharedEncryptionKey, &StandardClaims{}))
	}
...
func TestNewWithClaimsSigned(t *testing.T) {
	key := []byte("secret")
	type args struct {
		c Claims
		s *SignOpt
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := NewWithClaims(tt.args.c, tt.args.s, tt.args.e...)
			if (err != nil) != tt.wantErr {
				t.Errorf("NewWithClaims() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if tt.wantErr {
				return
			...
			fmt.Printf("got: %s\n", got)
			if got == "" {
				t.Errorf("NewWithClaims() returned empty token string")
			}
			tok, err := ParseSigned(got)
			if err != nil {
				t.Errorf("NewWithClaims() parse error = %v", err)
				return
			}
			fmt.Printf("tok: %#v\n", *tok)
			destCl := MapClaims{}
			tok.Claims(key, &destCl)
			fmt.Printf("got: %#v\n", destCl)
		})
	}
}

func TestNewWithClaimsSignedAndEncrypted(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tok, err := ParseSignedAndEncrypted(got)
			if err != nil {
				t.Errorf("NewWithClaims() parse error = %v", err)
				return
			}
