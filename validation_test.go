...
func TestOptionalDateClaims(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			"fail nbf",
			StandardClaims{NotBefore: NewNumericDate(time.Now())},
			ErrNotValidYet,
		},
		{
			"fail exp",
			StandardClaims{Expiry: NewNumericDate(epoch.Add(-7 * 24 * time.Hour))},
			ErrExpired,
		},
		{
			"fail iat",
			StandardClaims{IssuedAt: NewNumericDate(time.Now())},
